...
- (BOOL)supportsMethod:(NSString *)method atPath:(NSString *)path
{
    ...
    // Support web services through POST (not uploads)
    if ([method isEqualToString:@"POST"]) 
    {
        return [[HTTPServiceManager sharedInstance] serviceURLIsSupported:path]; 
    }
...
- (void)replyToHTTPRequest
{
	...
	// Respond properly to HTTP 'GET' and 'HEAD' commands (and 'POST' for web services)
	httpResponse = [[self httpResponseForMethod:method URI:uri] retain];
	
    NSLog(@"Sending response %@ %@ %@", httpResponse, uri, [self filePathForURI:uri allowDirectory:NO]);
	if (httpResponse == nil)
	{
		[self handleResourceNotFound];
		return;
...
- (NSObject<HTTPResponse> *)httpResponseForMethod:(NSString *)method URI:(NSString *)path
{
	...
	if (filePath && [[NSFileManager defaultManager] fileExistsAtPath:filePath isDirectory:&isDir] && !isDir)
	{
        ...
        //	return [[[HTTPAsyncFileResponse alloc] initWithFilePath:filePath forConnection:self] autorelease];
             ...
             [[HTTPServiceManager sharedInstance] serviceURLIsSupported:path])
    {
        // Check to see if the POST was for a web service instead
        NSString *SOAPMessage = [[NSString alloc] initWithData:bodyData encoding:NSUTF8StringEncoding];
        
        NSString *serviceResponse = [[HTTPServiceManager sharedInstance] SOAPresponseForServiceRequest:SOAPMessage forServiceURL:path];
        [SOAPMessage release];
        NSLog(@"serviceResponse = %@", serviceResponse);
        NSData *data = [serviceResponse dataUsingEncoding:NSUTF8StringEncoding];
...
- (void)prepareForBodyWithSize:(UInt64)contentLength
{
	...
	bodyData = [[NSMutableData alloc] initWithCapacity:contentLength];
}
